...
| Коллекция | Операция | Primary/Secondary | Обоснования |
|-----------|----------|-------------------|-------------|
| orders| Быстрое создание заказов с одновременным списанием остатков.|Primary | Операция создания + списание - нужна консистентность.|
| | Поиск истории заказов конкретного пользователя.|Secondary | Историю можем чичтать с реплики. Можем заложить до минуты. Не сильно критично. |
| | Отображение статуса заказа.|Secondary | Если нет сильрых ограничений, то и статус можно взять с реплики. Но время лучше заложить до 10 секунд.|
| | ...      |                   |             |
| products| Частые обновления остатков при покупках.|Primary | Нужна консистентность.|
| | Поиск товаров по категориям и фильтрация по диапазону цен.|Secondary | Отображение каталога (если нет требования по отображению точного количества остатков).|
| | Описание товара на странице продукта.|Secondary | Отображение описания (если нет требования по отображению точного количества остатков).|
| | ...      |                   |             |
| carts| Создание корзины, когда заходит гость или новый пользователь.|Primary | Операция создания|
| | Получение текущей корзины по фильтру { session_id, status:"active" } или { user_id, status:"active" }.|Primary | Нужна консистентность. Недостимо, чтобы у пользователя "скакали" товары в корзине".|
| | Добавление или замена товара в корзине.|Primary | Тоже самое, что и обоснование выше.|
| | Удаление товара из корзины.|Primary | Тоже самое, что и обоснование выше.|
| | Слияние гостевой корзины в пользовательскую, если пользователь залогинится.|Primary | Тоже самое, что и обоснование выше.|
| | Отметка корзины как заказанной.|Primary | Тоже самое, что и обоснование выше.|
| | ...      |                   |             |


# Задание 10. Миграция на Cassandra: модель данных, стратегии репликации и шардирования
Заказы, товары, корзины являются критически важными с точки зрения целостности и скорости обработки.

Применение Cassandra имеет смысл  для данных с большим количеством записей и это: заказы, товары, корзины. 

Кластерные ключи:
Заказы по Id пользователя (Равномерное распредление)
Товары по категории (Не совсем равномерное распределение, но список категорий конечный и эксклюзивный и может хорошо расширяться и перераспеделяться)
Корзины по Id пользователя (Равномерное распредление)

Восстановления:
Для заказов - Hinted Handoff (не очень частые чтения)
Для товаров - Read Repair (частые чтения)
Для корзины - Hinted Handoff (не очень частые чтения)